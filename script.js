...
const DomElement = function (select, hei, wid, back, font) {
    ...
    this.createElem = () => {
        const elem = String(this.selector)[0] === "." ? document.createElement("div") : document.createElement("p");
        String(this.selector)[0] === "." ? elem.innerHTML = `<div class = "${String(this.selector.slice(1))}">Класс данного блока - '${String(this.selector.slice(1))}'</div>` : elem.innerHTML = `<p id = "${String(this.selector.slice(1))}">Id данного параграфа - '${String(this.selector.slice(1))}'<p>`;
